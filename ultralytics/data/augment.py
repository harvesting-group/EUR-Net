    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            if "group_id" in labels:
                group_id.append(labels["group_id"])
        ...
        if group_id:
            final_labels["group_id"] = np.concatenate(group_id, 0)
        if instance_id:
            final_labels["instance_id"] = np.concatenate(instance_id, 0)
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        final_labels["cls"] = final_labels["cls"][good]
        final_labels['ripeness'] = final_labels['ripeness'][good]
        if "group_id" in final_labels:
            final_labels["group_id"] = final_labels["group_id"][good]
        if "instance_id" in final_labels:
            final_labels["instance_id"] = final_labels["instance_id"][good]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if "group_id" in labels and "group_id" in labels2:
            labels["group_id"] = np.concatenate([labels["group_id"], labels2["group_id"]], 0)
        if "instance_id" in labels and "instance_id" in labels2:
            labels["instance_id"] = np.concatenate([labels["instance_id"], labels2["instance_id"]], 0)
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if group_id is not None:
            labels["group_id"] = group_id[i]
        if instance_id is not None:
            labels["instance_id"] = instance_id[i]
        labels["img"] = img
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        group_id = labels1.get("group_id")
        instance_id = labels1.get("instance_id")
        h, w = im.shape[:2]
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            if group_id is not None:
                group_source = labels2.get("group_id", group_id)
                group_id = np.concatenate((group_id, group_source[[j]]), axis=0)
            if instance_id is not None:
                instance_source = labels2.get("instance_id", instance_id)
                instance_id = np.concatenate((instance_id, instance_source[[j]]), axis=0)
        ...
        if group_id is not None:
            labels1["group_id"] = group_id
        if instance_id is not None:
            labels1["instance_id"] = instance_id
...
class Format:
    ...
    def __call__(self, labels):
        ...
        group_id = labels.pop("group_id", None)
        instance_id = labels.pop("instance_id", None)
        instances = labels.pop("instances")
        ...
        if self.return_mask or self.return_ripeness:
            if nl:
                ...
                masks, instances, cls, ripeness, group_id, instance_id = self._format_segments(
                    instances, cls, ripeness, group_id, instance_id, w, h
                )
        ...
        if group_id is not None:
            group_id = np.full((nl,), group_id, dtype=np.int64) if np.isscalar(group_id) else np.asarray(group_id).reshape(-1)
            labels["group_id"] = torch.from_numpy(group_id).long() if nl else torch.zeros(nl, dtype=torch.long)
        if instance_id is not None:
            labels["instance_id"] = torch.from_numpy(instance_id.reshape(-1)).long() if nl else torch.zeros(nl, dtype=torch.long)
    ...
    def _format_segments(self, instances, cls, ripeness, group_id, instance_id, w, h):
        """
        ...
        if self.mask_overlap:
            ...
            if group_id is not None and not np.isscalar(group_id):
                group_id = group_id[sorted_idx]
            if instance_id is not None:
                instance_id = instance_id[sorted_idx]
        else:
            masks = polygons2masks((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        return masks, instances, cls, ripeness, group_id, instance_id
