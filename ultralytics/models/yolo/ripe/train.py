...
class RipenessTrainer(yolo.detect.DetectionTrainer):
    ...
    task_loss_names = ('box_loss', 'seg_loss', 'cls_loss', 'dfl_loss', 'ripe_loss')
    lightmoe_loss_names = (
        'corr_loss',
        'route_loss',
        'identity_loss',
    ...
    def get_validator(self):
        ...
        has_lightmoe = any(module.__class__.__name__ == 'LightMoEStem' for module in self.model.modules())
        self.loss_names = self.task_loss_names
        if self.args.light_single_aux and has_lightmoe:
            self.loss_names += self.lightmoe_loss_names
        return yolo.ripe.RipenessValidator(self.test_loader, save_dir=self.save_dir, args=copy(self.args))

    def preprocess_batch(self, batch):
        """Preprocess a batch and expose the current epoch to the ripeness criterion."""
